...
func (s *apiService) Start() {
	s.logger.Info().Msg("petstore-code server is starting up")
	...
	err := g.Wait()
	if err != nil {
		s.logger.Info().Msgf("error occured starting petstore-code server: %s", err.Error())
	}
	s.logger.Info().Msg("petstore-code server shutting down...")
}
...
func (s *apiService) Serve(ctx context.Context) error {
  ...
  loggedMux := s.logMiddleware(router)
	...
	go func() {
		s.logger.Info().Msgf("starting petstore-code server, address: %s", s.addr)
	...
	select {
	case <-ctx.Done():
		s.logger.Info().Msg("petstore-code server graceful shutdown started")
		server.Shutdown(ctx)
		s.logger.Info().Msg("petstore-code server successfully shutdown")
		return nil
	case err := <-errChan:
		s.logger.Info().Msgf("petstore-code server shutdown with error: %s", err.Error())
		return err
	}
}

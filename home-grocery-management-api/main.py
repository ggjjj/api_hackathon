...
from schemas import GroceryItem, BatchItems, Alert, ShoppingList
...
def add_item(item: GroceryItem, db: Session = Depends(get_db)):
    ...
    db.commit()
    db.refresh(db_item)
    return {"status": "success", "message": "Item added", "item": db_item}
...
def update_item(id: int, item: GroceryItem, db: Session = Depends(get_db)):
    ...
    db.commit()
    db.refresh(db_item)
    return {"status": "success", "message": "Item updated", "item": db_item}
...
def batch_add_items(batch: BatchItems, db: Session = Depends(get_db)):
    db_items = [GroceryItemDB(
        name=item.name,
        category=item.category,
        quantity=item.quantity,
        expiration_date=item.expirationDate
    ...
    for item in db_items:
        db.refresh(item)
    return {"status": "success", "message": "Items added", "items": [GroceryItem.from_orm(item) for item in db_items]}
...
def generate_shopping_list(minQuantity: Optional[int] = 1, category: Optional[str] = None, db: Session = Depends(get_db)):
    ...
    shopping_list = [
        ...
        for item in items if int(item.quantity.split()[0]) < minQuantity
    ]
